...
import { ActivityType, getCurrentActivity, listenToActivities } from '../activities/activitiesEntity'
...
export class ZonePollSystem {
  ...
  private close(zonePollEntity: Entity): void {
    ...
    closeZonePoll(zonePollEntity)
    this.clearZones()
    this.hideUIs()
    closeAllOpenedDoors()

    this.displayResults(zonePollEntity)
  }
  ...
  private displayResults(zonePollEntity: Entity): void {
    const pollState = ZonePollState.getOrNull(zonePollEntity)
  ...
  private start(zonePollEntity: Entity): void {
    ...
    const zones = [
      { color: Color4.Red(), position: Vector3.create(13.1, 0.2, 6.64) },
      { color: Color4.Green(), position: Vector3.create(10.54, 0.2, 3.21) },
      { color: Color4.Yellow(), position: Vector3.create(2.83, 0.14, 6.64) },
      { color: Color4.Blue(), position: Vector3.create(6, 0.18, 3.4) }
    ] as const
    ...
    zonePollState.options.forEach((option, index) => {
      ...
      const optionZone = new OptionZone(zone.position, zone.color, index, zonePollEntity, this.gameController)
